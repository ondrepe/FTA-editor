 ...
 * @author ondrepe
 */
@XmlRootElement(name="svgExclusiveOrGate")
public class SvgExclusiveOrGate extends SvgGroupObject {

    private Element group;

    public SvgExclusiveOrGate() {
    }
    
    public SvgExclusiveOrGate(int x, int y) {
        super(x, y);
    }
    ...
    public void init() {
        ...
        String data = "M " + String.valueOf((WIDTH / 2) - RADIUS) + "," + String.valueOf(HEIGHT + DEFUAL_STEP_VALUE + 55) + " " +
                ...
                " c 1,-17 16,-29 25,-30 c 11,1 25,16 25,30 l 0,25 z";
        ...
        Element orLine = getDocument().createElementNS(SVG_NS, SVG_TYPE_PATH);
        String lineData = "M " + String.valueOf((WIDTH / 2) + RADIUS) + "," +
                String.valueOf(String.valueOf(HEIGHT + DEFUAL_STEP_VALUE + 55))
                + " c -15,-15 -35,-15 " + String.valueOf(-(2 * RADIUS)) + ",0";
        
        orLine.setAttributeNS(null, ATT_DATA, lineData);
        orLine.setAttributeNS(null, ATT_STROKE_WIDTH, String.valueOf(LINE_WIDTH));
        orLine.setAttributeNS(null, ATT_STROKE, COLOR_BLACK);
        orLine.setAttributeNS(null, ATT_FILL, COLOR_WHITE);
        innerGroup.appendChild(orLine);
