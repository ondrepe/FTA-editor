 ...
 * @author ondrepe
 */
@XmlRootElement(name="svgInhibitGate")
public class SvgInhibitGate extends SvgGroupObject {

    private Element group;

    public SvgInhibitGate() {
    }

    public SvgInhibitGate(int x, int y) {
        super(x, y);
    }
    ...
    public void initElement() {
        ...
        Element inhibit = getDocument().createElementNS(SVG_NS, SVG_TYPE_POLYGON);
        
        double delta = Math.sqrt((RADIUS * RADIUS) - ((RADIUS / 2) * (RADIUS / 2)));
        String points = String.valueOf(getPosition().x + (WIDTH / 2)) + "," + String.valueOf(getPosition().y + HEIGHT + DEFUAL_STEP_VALUE) + " " +
                String.valueOf(getPosition().x + (WIDTH / 2) + delta) + "," + String.valueOf(getPosition().y + HEIGHT + DEFUAL_STEP_VALUE + (RADIUS / 2)) + " " +
                String.valueOf(getPosition().x + (WIDTH / 2) + delta) + "," + String.valueOf(getPosition().y + HEIGHT + DEFUAL_STEP_VALUE + (RADIUS / 2) + RADIUS) + " " +
                String.valueOf(getPosition().x + (WIDTH / 2)) + "," + String.valueOf(getPosition().y + HEIGHT + DEFUAL_STEP_VALUE + (2 * RADIUS)) + " " +
                String.valueOf(getPosition().x + (WIDTH / 2) - delta) + "," + String.valueOf(getPosition().y + HEIGHT + DEFUAL_STEP_VALUE + (RADIUS / 2) + RADIUS) + " " +
                String.valueOf(getPosition().x + (WIDTH / 2) - delta) + "," + String.valueOf(getPosition().y + HEIGHT + DEFUAL_STEP_VALUE + (RADIUS / 2));
        
        inhibit.setAttributeNS(null, ATT_POINTS, points);
        inhibit.setAttributeNS(null, ATT_STROKE_WIDTH, String.valueOf(LINE_WIDTH));
        inhibit.setAttributeNS(null, ATT_STROKE, COLOR_BLACK);
        inhibit.setAttributeNS(null, ATT_FILL, COLOR_WHITE);
        group.appendChild(inhibit);
